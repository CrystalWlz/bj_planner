...
const parameterExplanations: Record<string, string> = {
  ...
  被裁员成员: "选择职业冲击压力情景作用到哪个收入成员。公开模板默认关闭该情景。",
  第一成员出生年月: "优先用于把裁员年龄和退休年龄换算成真实月份。公开仓库默认留空，请在本机初始化时填写。",
  第二成员出生年月: "优先用于把第二位成员的退休年龄换算成真实月份。公开仓库默认留空，请在本机初始化时填写。",
  第一成员当前年龄: "仅在出生年月缺失或格式错误时兜底使用；公开模板使用中性占位值。",
  第二成员当前年龄: "仅在出生年月缺失或格式错误时兜底使用；公开模板使用中性占位值。",
  ...
  return (
    <div className="page-stack">
      ...
      <section className="form-panel setup-guide">
        ...
        <p className="field-hint">
          公开版本只保留北京政策规则和空白家庭模板，不内置任何私人收入、资产、贷款或家庭成员信息。首次使用建议按下面顺序填写，完成后点击“保存本地”写入你自己的本机数据库。
        </p>
