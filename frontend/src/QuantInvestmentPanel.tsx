...
export function QuantInvestmentPanel({ householdId }: { householdId: string }) {
  ...
  const [postTradeReviewNote, setPostTradeReviewNote] = useState("");
  ...
  const reviewPostTradeRisk = () => runAction(async () => {
    if (!portfolio?.post_trade_risk_state_hash) throw new Error("当前没有可复核的事后风控状态。 ");
    const reviewNote = postTradeReviewNote.trim();
    if (!reviewNote) throw new Error("请填写本次事后风控复核说明。 ");
    await reviewQuantPostTradeRisk(householdId, portfolio.post_trade_risk_state_hash, reviewNote);
    setPostTradeReviewNote("");
  });
  ...
  return (
    <section className="form-panel quant-investment-panel">
      ...
      {portfolio ? <div className="setting-group"><div className="section-header"><strong className="setting-group-title">对账与事后风控</strong><button className="secondary-button" type="button" onClick={() => void reconcilePaperBroker()} disabled={busy}><ShieldCheck size={16} /> 核验模拟账本</button></div><div className="metric-grid"><Metric label="对账日期" value={displayedReconciliation?.data.reconciliation_date || "尚未运行"} /><Metric label="对账范围" value={displayedReconciliation?.data.adapter === "qmt" ? "QMT 只读" : "模拟盘"} /><Metric label="对账结果" value={!displayedReconciliation ? "待核验" : displayedReconciliation.data.matched ? "一致" : "存在差异"} tone={!displayedReconciliation ? undefined : displayedReconciliation.data.matched ? "good" : "bad"} /><Metric label="对账复核状态" value={!displayedReconciliation ? "无需复核" : displayedReconciliation.data.review_status === "pending" ? "待人工复核" : displayedReconciliation.data.review_status === "resolved" ? "已复核" : "无需复核"} tone={displayedReconciliation?.data.review_status === "pending" ? "bad" : "good"} /><Metric label="事后风控复核" value={portfolio.post_trade_review_status === "pending" ? "待人工复核" : portfolio.post_trade_review_status === "reviewed" ? "已按当前状态复核" : "无需复核"} tone={portfolio.post_trade_review_status === "pending" ? "bad" : "good"} /></div>{portfolio.post_trade_risk_issues.length ? <div className="warning-list">{portfolio.post_trade_risk_issues.map((issue) => <span key={`${issue.code}-${issue.order_id}-${issue.instrument_id}`} className={issue.severity === "freeze" && portfolio.frozen ? "error-text" : undefined}>{issue.message}</span>)}</div> : null}{portfolio.post_trade_review_status === "pending" ? <div className="form-grid two"><label className="field"><span>事后风控复核说明</span><input value={postTradeReviewNote} onChange={(event) => setPostTradeReviewNote(event.target.value)} placeholder="确认已核对风险原因、持仓和现金" /></label><button className="secondary-button" type="button" onClick={() => void reviewPostTradeRisk()} disabled={busy || !postTradeReviewNote.trim()}><ShieldCheck size={16} /> 确认当前风险状态</button></div> : null}</div> : null}
