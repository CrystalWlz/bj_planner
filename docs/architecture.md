...
- `backend/app/calculator.py`  
  后端计算主入口。负责税务、收入阶段、支出阶段、投资、贷款、购房策略、购车策略、养娃策略、幸福指数、账户曲线、事件时间线和导出所需数据。这个文件仍然较大，新增纯算法逻辑时优先下沉到 `domain/` 或专门模块。
...
- `tax_monthly_points` / `tax_year_summaries`：税务月度与年度结果。
- `plan_events`：事件时间线。
...
- 车辆需要能源类型、北京小客车指标、候选车源、经销商金融方案、保险、保养、报废或更新周期等参数。
...
```text
家庭财务 -> 购车计划 -> 购房计划 -> 理财计划 -> 养娃计划 -> 税务 -> 可视化 -> 政策规则 -> 导出方案
```
...
- 税务：专项附加扣除、年终奖、个人养老金、自由职业税务、投资税务等。
...
- `backend/app/calculator.py` 仍承担很多职责，应继续把纯算法、策略生成、账户推演、税务推演分模块。
