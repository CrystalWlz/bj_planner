...
- `backend/app/market_data.py`、`market_calendar.py`、`domain/quant_investment.py`、`domain/quant_backtest.py`、`domain/paper_portfolio.py`、`strategies/quant_investment.py`、`broker_adapters.py`
  量化定投的独立边界。`market_data.py` 只通过本机 `TUSHARE_TOKEN` 拉取日频行情；环境变量优先，未设置时仅读取用户私有的 `%APPDATA%\house-planner\tushare.env`，绝不写入项目、SQLite 家庭数据或前端。行情数据集保存来源接口、抓取时间、交易日历、复权口径、价格日、净值估值日/可得日、完整性、内容哈希和版本，数据库按内容哈希保留不同版本，旧回测引用的快照不会被新数据覆盖。交易所 ETF 与港股分别尝试把 `fund_adj`、`hk_adjfactor` 的完整因子合并为因果后复权序列；权限不足或任一价格日缺因子时整段保留原始价、标记 `adjustment=none` 并把降级警告返回前端，不允许部分日期混用口径。`strategies/quant_investment.py` 用稳定的 `SignalModel → PortfolioConstructor → PreTradeRiskManager → ExecutionPlanner` 编排提案；默认只有月度回撤信号、35/65 权益/防御配置和整手模拟执行，研究优化器默认关闭。防御资产必须由用户手工加入，未配置或受集中度约束的防御额度保留为模拟现金。集中度按当前持仓加本批次计划买入后的整个模拟组合校验，分母包含本次外部投入，买单以含费用金额占用额度；再平衡卖出不受买入集中度限制，再平衡买入先扣除计划卖出暴露。季度月份仅在权益偏离达到阈值时生成再平衡卖出/买入，模拟现金和可用持仓在成交 API 再校验。`domain/quant_backtest.py` 按实际共同交易日推进，在信号后的下一可交易日成交，并处理防御资产、季度再平衡、停牌、限购、最小交易单位、费率、滑点和净值滞后；除静态 35/65 外还输出现金与 100% 权益基准、24/12 月滚动样本外分段。Recorder 风格运行记录保存策略/标的池/数据版本、参数、成本、区间、指标和警告。模拟订单状态与不可变成交事件在同一 SQLite 事务落库；外部预算由订单 schema v2 的 `cash_contribution_amount` 只注入一次，整手成交后的余款留在模拟现金，旧订单在数据库启动时就地迁移。未成交订单取消按 `proposed → cancel_requested → cancelled` 转换，每一步写入 `paper_investment_order_events`；适配器未确认时保持可重试的取消请求，不能直接写成已取消。`PaperPortfolioLedger` 再派生 `MonthlyLedgerEntry → AccountSnapshotPoint → MonthlyVisualizationDetail`，并通过 `AffordabilityResult.paper_portfolio` 进入主计算结果，前端不重算持仓或盈亏。事前风控覆盖现金安全垫、未来 24 个月支出/目标、交易资格、QDII 溢价、集中度和订单限额；事后风控输出结构化 `PostTradeRiskIssue`，回撤、成交偏离、净值过期与对账差异只冻结新增。`BrokerAdapter` 固定提供提交、撤单、订单/持仓/现金查询和对账，`LocalFirstBrokerGateway` 使用数据库校验器核对 `local_order_id + client_order_id` 后才允许调用适配器。对账运行把订单、持仓和现金规范化为双侧哈希并保存到 `broker_reconciliation_runs`；未复核 mismatch 是锁存状态，不能被后续 matched 自动覆盖。`PaperBrokerAdapter` 只核验本地模拟状态，`QmtBrokerAdapter` 在券商书面确认和只读对账阶段完成前始终不可执行。行情、标的、政策、新成交、订单取消、事后风控复核和对账复核会递增 `HouseholdData.quant_investment_data_version` 并进入 affordability input cache 指纹。详细流程与凭据边界见 `docs/investment-quant-plan.md`。

  行情快照以 `instrument_id + dataset_hash` 作为不可变内容版本键。哈希计算排除 `fetched_at`、`dataset_hash` 和派生的 `data_version`，API 返回的完整快照原样重放仍定位到同一内容版本。同哈希刷新只复用原记录，不改写 `data`、抓取时间或更新时间；只有新哈希才新增版本、递增 `HouseholdData.quant_investment_data_version` 并使相关缓存失效。这保证旧回测引用的快照 ID 和 Recorder 指纹在后续刷新后仍可复现。行情 schema v3 对缺失 QDII 公告日默认使用估值日后一天；v2 中估值日与可得日相同的歧义记录也保守顺延。提案、回测和 `PaperPortfolioLedger` 事后风控使用同一个有效净值可得日函数。

  `market_calendar.py` 独立封装沪深 `trade_cal` 与港股 `hk_tradecal`。接口无权限或不可用时回退到实际价格日期，并把日历来源和警告写入快照；日历开放但缺少价格的日期按停牌处理。跨市场风险篮子必须按共同价格日期对齐，不能按序列长度错位拼接；QDII 净值日龄优先按快照开放交易日计数。模拟订单默认在自身 `expected_trade_date` 成交，人工回填只允许相同或更晚的有效 ISO 日期，避免成交事件早于信号时钟。模拟账户用资金流中性的单位净值计算当前/历史最大回撤，达到冻结阈值后只暂停新增。生产 `Fixed3565PortfolioConstructor` v3 固定在权益池、防御池内等权分配且不读取 `research_strategy`；最小方差研究策略只在回测中按每个信号日前最近 24 个月数据重估权重，再用于下一共同可交易日，测试窗口和未来数据不会进入拟合。该策略默认关闭，也不进入生产提案。量化页面把后端 `QuantBacktestResult` 中的回撤风控策略与静态 35/65 按终值、CAGR、年化波动、最大回撤、换手率、累计费用和最差现金余额同表比较；现金与 100% 权益定投只作为补充基准，前端不反推指标。

  `PaperPortfolioLedger` 按 `executed_date + order_id` 重放成交，使用每月末最近已确认的原始 `close` 派生现金、持仓市值和已实现/未实现盈亏；`adjusted_close` 仅用于研究信号和回测，不能进入生产成交报价或模拟持仓估值。账本版本为 `paper-portfolio-v3`：每笔成交先按当前可用持仓生成实际应用成交，卖出超出持仓时同步截断现金、手续费、卖出金额和盈亏，输出 `position_oversell` 事后风控并冻结新增提案等待人工核对；订单与成交完整性也在账本摘要构建时前置校验，异常会形成 `paper_ledger_integrity` 冻结，不必等待手动对账接口。冻结不会自动补仓、卖出或改写原始成交。即使月份没有成交，也输出连续估值快照。组合摘要支持显式 `as_of_date`，历史估值只重放该日及以前的成交、价格、净值可得性与回撤，并派生该自然月逐标的含费用买入额。成交 API 同样按拟成交日回放已有现金和持仓，禁止历史卖出借用未来买入或模拟现金买入借用未来卖出。`_attach_paper_portfolio` 同步重建 `paper_quant` 的摘要、持仓、快照、流水、事后风控表和文字导出，缓存命中时先去重旧导出。量化页面直接使用 `account_snapshots` 绘制模拟现金、持仓市值和账户权益曲线，并用 `visualization_details` 展示所选月份的顾问说明、资金驱动和解释项；前端只负责月份选择、格式化与图表渲染，不接触任何近似余额、净值、盈亏、费用或回撤计算，没有快照时不渲染空图。
  需要人工确认的事后风控异常会按当前异常字段生成 `post_trade_risk_state_hash`，复核记录只接受与当前状态完全一致的哈希；复核后不改写成交历史，只解除这一状态的新增冻结。历史 `as_of_date` 回放只使用不晚于估值日的复核记录，今天的复核不能倒灌到过去。新成交引入新的异常、回撤进一步恶化或其它异常字段改变哈希后会再次冻结。`broker_dispatch_uncertain` 和 `reconciliation_mismatch` 不属于普通复核范围，必须继续通过券商动作/对账专用流程解除。

  Paper 买入在创建 outbox 前及发送前复用 `PreTradeRiskManager` v5。无政策、非手工池标的或无可追溯行情直接拒绝；港股通资格、QDII 溢价/净值、单笔上限、成交后单标的/单市场集中度和月累计申购额度都按实际成交金额加费用复核。当月已成交买入与创建顺序更早的未完成买入 outbox 合并占额，外部投入同步进入组合分母。卖出不受新增买入门禁影响，但仍校验执行日和成交日可用持仓。
  ...
  `broker_order_dispatches` 是券商动作的持久化 outbox。请求参数先以 `pending` 状态不可变落库，创建时间在取得 SQLite 写锁后生成，使并发预留顺序与事务提交顺序一致。发送前风险复核记录家庭量化数据版本，Gateway 在认领事务中用该版本做比较交换；风险状态被并发订单、成交、行情或政策改变时拒绝认领过期结果。尚未发送且复核失败的 `pending` 动作可原子删除并释放预留，任何已认领、已确认、结果不确定或人工复核后的动作都不能走该删除路径。Gateway 原子认领为 `dispatching` 后才调用适配器，响应再保存为 `acknowledged`；调用异常或进程中断形成的 `dispatching/uncertain` 会进入 `PostTradeRiskIssue` 并冻结新增，不能自动重发。已确认响应与成交事务之间中断时，模拟成交 API 从 outbox 响应补齐唯一成交。仍在五分钟发送窗口内的 `dispatching` 不允许复核为可重试；只有同一适配器、晚于异常或失联窗口的一致对账和人工复核说明同时存在，动作才转为 `retryable`，并继续使用原请求及原 `client_order_id`。API 派生 `retry_eligible`、可用对账记录和阻塞原因，前端只展示后端判定，不自行计算发送窗口或对账资格。outbox 状态变化与家庭量化数据版本在同一 SQLite 事务更新，使缓存不会掩盖发送异常。模拟盘对账还会校验订单状态与不可变成交事件的一一对应、标的/提案/client_order_id 一致性和重复成交；完整性异常同样冻结新增。QMT 仍保持禁用；后续接入时复用该边界，不另建绕过 outbox 的发送路径。
...
- `export_sheets`：结构化导出表格，由后端 `reporting.py` 从核心对象索引、ledger/snapshot 和专项账户序列生成，前端只负责序列化为 CSV。其中“核心对象与账户概念”表直接复用 `account_concepts` 的对象数量、当前余额和月流量，“核心对象分组摘要”表复用 `core_object_groups` 的后端分组口径，量化模拟账户另有 `paper_quant` 摘要、持仓、快照、流水和事后风控表，避免导出时再从前端页面字段反推账户、资产和贷款口径。
- `export_texts`：结构化文字导出，由后端 `reporting.py` 从策略、事件、账户快照和专项账户序列生成；量化模拟账户额外输出 `quant-paper-portfolio.txt`，前端只负责下载文本。
...
- `paper_investment_orders` / `paper_investment_fills`：模拟交易意图和不可变成交事件。订单 schema v2 显式保存模拟现金投入；旧订单迁移只更新 JSON 数据，不重建表，不改变订单 ID、状态或成交关联。
- `paper_investment_order_events`：订单取消请求与适配器确认的不可变状态事件；同一订单同一事件类型唯一，用于幂等重试和审计。
- `post_trade_risk_reviews`：普通事后风控的不可变人工复核记录，按家庭和精确风险状态哈希唯一保存异常代码、复核时间和说明；它不能覆盖券商发送不确定或对账差异的专用复核流程。
- `broker_reconciliation_runs`：不可变对账运行和人工复核状态，保存适配器、运行日期、双侧状态哈希、订单/持仓/现金摘要与差异；未复核差异会锁存冻结新增提案。
...
3. 在 `storage/normalization.py` 或 `database.py` 的专用幂等迁移中把旧记录一次性转成最新结构。
