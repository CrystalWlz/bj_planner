...
- `backend/app/market_data.py`、`market_calendar.py`、`domain/quant_investment.py`、`domain/quant_backtest.py`、`domain/paper_portfolio.py`、`strategies/quant_investment.py`、`broker_adapters.py`
  ...
  行情快照以 `instrument_id + dataset_hash` 作为不可变内容版本键。同哈希刷新只复用原记录，不改写 `data`、抓取时间或更新时间；只有新内容哈希才新增版本、递增 `HouseholdData.quant_investment_data_version` 并使相关缓存失效。这保证旧回测引用的快照 ID 和 Recorder 指纹在后续刷新后仍可复现。
