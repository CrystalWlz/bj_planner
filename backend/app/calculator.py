...
def _car_monthly_cash_cost_at(plan: CarPlanData, car_loan: CarLoanSummary, month: int) -> float:
    ...
    if first_purchase_month is None:
        ...
        payment = 0.0
        if month_after_car <= car_loan.total_months:
            payment = (
        ...
        base_cost = payment + car_loan.monthly_cash_operating_cost
    ...
    second_payment = 0.0
    if month_after_second <= second_loan.total_months:
        second_payment = (
    ...
    return base_cost + second_payment + second_loan.monthly_cash_operating_cost
...
def _min_down_payment_ratio(household: HouseholdData, uses_provident_loan: bool, rules: RulePackData) -> float:
    first_home = household.existing_home_count == 0
    if first_home:
        commercial_ratio = float(rules.params.get("first_home_commercial_min_down_payment_ratio", 0.15))
        provident_ratio = float(rules.params.get("first_home_provident_min_down_payment_ratio", 0.20))
    else:
        commercial_ratio = float(rules.params.get("second_home_commercial_min_down_payment_ratio", 0.20))
        provident_ratio = float(rules.params.get("second_home_provident_min_down_payment_ratio", 0.25))
    return max(commercial_ratio, provident_ratio if uses_provident_loan else 0)


def _provident_policy_bonus(scenario: ScenarioData, rules: RulePackData) -> float:
    params = rules.params
    bonuses: list[float] = [0.0]
    if scenario.green_building_level == "two_star":
        bonuses.append(float(params.get("provident_green_two_star_bonus", 200_000)))
    elif scenario.green_building_level == "three_star":
        bonuses.append(float(params.get("provident_green_three_star_bonus", 300_000)))

    prefab_bonus_by_level = {
        "A": float(params.get("provident_prefab_a_bonus", 100_000)),
        "AA": float(params.get("provident_prefab_aa_bonus", 200_000)),
        "AAA": float(params.get("provident_prefab_aaa_bonus", 300_000)),
    }
    bonuses.append(prefab_bonus_by_level.get(scenario.prefab_building_level, 0.0))
    if scenario.is_ultra_low_energy_building:
        bonuses.append(float(params.get("provident_ultra_low_energy_bonus", 400_000)))
    return min(max(bonuses), float(params.get("provident_policy_bonus_cap", 400_000)))
...
def _provident_loan_cap(
    ...
    *,
    ...
    return min(base_maximum_cap + bonus, policy_year_cap + bonus), bonus
...
def _provident_loan_years(household: HouseholdData, scenario: ScenarioData, rules: RulePackData) -> tuple[int, list[str]]:
    params = rules.params
    requested_years = int(scenario.loan_years)
    max_policy_years = int(params.get("provident_max_loan_years", 30))
    max_borrower_age = int(params.get("provident_max_borrower_age", 68))
    safety_deduction = int(params.get("provident_property_age_safety_deduction_years", 3))

    candidates: list[tuple[int, str]] = [
        (requested_years, f"按手动设定贷款年限 {requested_years} 年测算"),
        (max_policy_years, f"北京公积金贷款最长 {max_policy_years} 年"),
    ]

    age_limited_years = max_borrower_age - household.borrower_age
    candidates.append(
        (
            age_limited_years,
            f"借款申请人年龄 {household.borrower_age} 岁，可贷至 {max_borrower_age} 岁",
        )
    )

    if _is_second_hand_property(scenario):
        if scenario.is_old_community_renovated and scenario.remaining_land_use_years is not None:
            land_limited_years = scenario.remaining_land_use_years - safety_deduction
            candidates.append(
                (
                    land_limited_years,
                    f"已完成老旧小区改造，按剩余土地使用年限 {scenario.remaining_land_use_years} 年减 {safety_deduction} 年",
                )
            )
        else:
            if scenario.building_structure == "steel_concrete":
                total_life_years = int(params.get("provident_steel_concrete_total_life_years", 60))
                structure_label = "钢混结构"
            else:
                total_life_years = int(params.get("provident_brick_mixed_total_life_years", 50))
                structure_label = "砖混结构" if scenario.building_structure == "brick_mixed" else "未知结构按砖混结构保守测算"
            property_limited_years = total_life_years - scenario.building_age_years - safety_deduction
            candidates.append(
                (
                    property_limited_years,
                    f"二手房{structure_label}按 {total_life_years} 年减房龄 {scenario.building_age_years} 年再减 {safety_deduction} 年",
                )
            )

    raw_min_years = min(years for years, _ in candidates)
    effective_years = max(1, raw_min_years)
    binding_reasons = [reason for years, reason in candidates if years == raw_min_years]
    if raw_min_years < 1:
        binding_reasons.append("政策年限测算低于 1 年，系统按 1 年作为压力测算下限")
    if effective_years < requested_years:
        return effective_years, binding_reasons
    return effective_years, [f"公积金贷款年限未被年龄或房龄收紧，按 {effective_years} 年测算"]
...
def _post_purchase_monthly_pf_withdrawal(
    *,
    ...
    if not bool(rules.params.get("provident_monthly_withdrawal_after_purchase_enabled", True)):
        return 0.0, "disabled"
...
def _post_purchase_cash_stress(
    *,
    ...
    for absolute_month in range(purchase_month + 1, purchase_month + horizon_months + 1):
        ...
        monthly_pf_withdrawal, _ = _post_purchase_monthly_pf_withdrawal(
...
def build_purchase_plan_analyses(
    ...
    *,
    ...
    for name, description, target_commercial, use_min_down, use_manual_mix, use_micro_strategy in variant_specs:
        ...
        monthly_pf_withdrawal, monthly_pf_withdrawal_mode = _post_purchase_monthly_pf_withdrawal(
        ...
        if not renovation_included_upfront and scenario.renovation_cost > 0:
            post_purchase_renovation_monthly_saving = max(0.0, post_purchase_cash_flow_with_pf)
        ...
        happiness_breakdown = [
            ...
            },
            {
                ...
                "weight": 0.16,
                "note": f"含买后公积金提取后月结余 {round(post_purchase_cash_flow_with_pf)}。",
            },
            {
                "name": "负债压力",
        ...
        analyses.append(
            PurchasePlanAnalysis(
                ...
                provident_repayment_method=_provident_repayment_method(scenario),  # type: ignore[arg-type]
                ...
                provident_extraction_notes=[
                    ...
                    "购房约定提取按后续月缴存额估算；若规则包改为 loan_offset，则按公积金贷款月供上限估算冲还贷的月均改善。",
                    "冲还贷生效时，原有购房、租房提取事项及已办理约定提取会同步终止，且冲还贷期间不再受理新的住房公积金提取业务。",
                    f"当前月度提取估算模式：{monthly_pf_withdrawal_mode}。",
                ],
...
def build_yield_sensitivity(
    ...
    *,
    ...
    for annual_return in [0.015, 0.025, 0.035]:
...
def calculate_affordability(
    ...
    *,
    ...
    if stress_name is None:
        result.stress_tests = build_stress_tests(household, scenario, rules)
    return result
...
def build_stress_tests(
    ...
    for name, stress_household, stress_scenario in cases:
